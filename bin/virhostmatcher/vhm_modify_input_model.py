...
parser.add_option("-s", "--FaDir", action = "store", type = "string", dest = "FaDir",
									help = "the directory to the folder containing bacteria virus fasta files")
parser.add_option("-p", "--FaDir_p", action = "store", type = "string", dest = "FaDir_p",
									help = "the directory to the folder containing virus fasta files if input pair")
...
parser.add_option("-k", "--kind", action = "store", type = "string", dest = "kind",default='p',help = "b:input bacteria,p:input phage,pair:bacteria and phage")
...
## name length ##
nameLen = 193 - len(options.outDir)
...
if options.hostTaxaFile is None :
	...
	hostFaList = os.listdir(options.FaDir)
...
if options.kind == 'b':
	...
	for currentFileName in hostFaList:
		if currentFileName.startswith('.'):
			continue
		if os.path.isdir(os.path.join(options.FaDir, currentFileName)):
		...
		for w in range(1, (kmax + 1)):
			currentFilePath = os.path.join(options.FaDir, currentFileName)
		...
		sys.stdout.write(" (Average time for counting kmers for one host: " + str(
			round((end_time - start_time) / count, 4)) + "s) \n")
		sys.stdout.write(" (ETR for counting kmers for hosts: " + str(
			round((end_time - start_time) / count * (len(hostFaList) - count), 4)) + "s) \n")
		filelog.write(" (Average time for counting kmers for one host: " + str(
			round((end_time - start_time) / count, 4)) + "s) \n")
		filelog.write(" (ETR for counting kmers for hosts: " + str(
			round((end_time - start_time) / count * (len(hostFaList) - count), 4)) + "s) \n")

	hostListFileWrite.close()
	virusListFile = options.listfile

elif options.kind == 'p':
	virusFaList = os.listdir(options.FaDir)
	start_time = time.time()
	count = 0
	for currentFileName in virusFaList:
		...
		count += 1
		# sys.stdout.write(str(end_time - start_time) + "s for " + str(count) + " seqs \n")
		sys.stdout.write(" (Average time for counting kmers for one virus: " + str(
			round((end_time - start_time) / count, 4)) + "s) \n")
		sys.stdout.write(" (ETR for counting kmers for viruses: " + str(
			round((end_time - start_time) / count * (len(virusFaList) - count), 4)) + "s) \n")
		filelog.write(" (Average time for counting kmers for one virus: " + str(
			round((end_time - start_time) / count, 4)) + "s) \n")
		filelog.write(" (ETR for counting kmers for viruses: " + str(
			round((end_time - start_time) / count * (len(virusFaList) - count), 4)) + "s) \n")

	virusListFileWrite.close()
	hostListFile = options.listfile
	print(hostListFile)
	#hostListFile = '/zrom1/ganrui/PHIS/consensus_version/data/temp_phage/hostList'

else:
	#################### 1: count kmer and prepare list files ############################
	#sys.stdout.write("Step 1: counting kmers \n")
	virusFaList = os.listdir(options.FaDir_p)	
	hostFaList = os.listdir(options.FaDir)
	start_time = time.time()
	count = 0
	for currentFileName in virusFaList :
		if currentFileName.startswith('.') :
			continue
		if os.path.isdir(os.path.join(options.FaDir_p, currentFileName)) :
			sys.stderr.write( "ERROR: zero bytes of file " + currentFileName + "\n")
			filelog.write( "ERROR: zero bytes of file " + currentFileName + "\n")
			sys.exit(0)
		if len(currentFileName) > nameLen :
		...
		filelog.write("Step 1: counting kmers for virus " + currentFileNameS + "\n")
		for w in range(1, (kmax+1)) :
			currentFilePath = os.path.join(options.FaDir_p, currentFileName)
