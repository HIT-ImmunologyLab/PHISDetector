...
vector<double> D2C2computeNGSOnlyd2star(int ZI, int k, KMERINFO* speciesKmerInfoA, KMERINFO* speciesKmerInfoB)
{
	// compute D2 statistics
	SCIENTIFIC_NUMBER D2star;
	D2star.value = 0; D2star.factor = 0;
	SCIENTIFIC_NUMBER C2star_below[2];
	C2star_below[0].value = 0; C2star_below[0].factor = 0;
	C2star_below[1].value = 0; C2star_below[1].factor = 0;
...
void computeMultiStatsReturnOnlyd2star(int ZI, int k, KMERINFO* speciesKmerInfoA, KMERINFO* speciesKmerInfoB, SPECIESINFO speciesInfoA, SPECIESINFO speciesInfoB, vector<string>& measureNames, vector<double>& measureValues)
{
	
	// 1. compute the D2 statistics
	string C2StatNames[3] = {"d2star"} ;
	//cout << "== compute the D2 statistics, consider complementary == " << endl;
	vector<double> C2NGSvalues = D2C2computeNGSOnlyd2star(ZI, k, speciesKmerInfoA, speciesKmerInfoB);
	//printFour(D2C2values);
  ...
  // load the kmer count hashtables of the first file list (the fat list)
	...
	for(int IDB = 0; IDB < speciesInfoListB.size(); IDB++)
	{
		...
		for(int IDA = 0; IDA < speciesInfoListA.size(); IDA++)
		{
			...
			measureNames->clear();
			computeMultiStatsReturnOnlyd2star(ZI, k, speciesKmerInfoA, speciesKmerInfoB, speciesInfoA, speciesInfoB, (*measureNames), (*measureValues));
